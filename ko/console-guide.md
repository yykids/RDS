...
> [참고] DB 파일 암호화 기능 사용시 성능이 다소 감소될 수 있습니다.
...
* 고가용성 인스턴스가 실제 장애로 인하여 장애조치가 수행되었을 경우, 변경된 새 Master 인스턴스는 기존 Master 인스턴스의 백업을 승계하지 않습니다.
...
* RDS에서는 모든 백업을 수행한 뒤, 생성한 백업을 자체적인 Object Stroage로 업로드하여 보관합니다.
* 자동백업에 한하여, 원본 인스턴스의 Data 볼륨 사이즈 만큼의 백업 용량을 무료로 제공합니다.
...
* 자동 백업은 원본 인스턴스가 모두 삭제될 경우 같이 삭제됩니다.

#### 수동 백업

* 자동 백업 이외에 언제든지 수동으로 백업을 할 수 있습니다.
* 수동 백업은 명시적으로 삭제 하지 않는한 지워지지 않습니다.
...
* 사용자 데이터가 저장되는 Database의 파일 및 백업 파일을 암호화합니다.

> [참고] 실시간으로 암호화를 수행하므로, DB 인스턴스의 성능이 다소 감소할 수 있습니다.

#### 제약 사항

* DB 파일 암호화 기능을 사용하지 않는 인스턴스로부터 복원이나 복제시 DB 파일 암호화 기능을 켤 수 없습니다.
* DB 파일 암호화 기능을 사용하는 인스턴스로부터 복원이나 복제시 DB 파일 암호화 기능을 끌 수 없습니다.
